...
## AI environment (Claude Code and OpenCode are optional)

This file is named `CLAUDE.md` for historical reasons. **The same rules apply** if you use **Cursor**, **ChatGPT / Codex in the IDE**, **GitHub Copilot**, or any other coding agent that reads this repository. You do **not** need to install [Claude Code](https://docs.anthropic.com/en/docs/claude-code) or [OpenCode](https://opencode.ai): open the project in an AI-enabled editor and follow these instructions. Slash commands like `/career-ops` are conveniences in those tools; without them, use natural language (e.g. “evaluate this JD using career-ops oferta mode”) and load `modes/*.md` as documented.
...
AI-powered job search automation: pipeline tracking, offer evaluation, CV generation, portal scanning, batch processing. Usable from **Cursor**, **Claude Code**, **OpenCode**, or any assistant that follows `CLAUDE.md` and `modes/`.
...
If you use [OpenCode](https://opencode.ai), the following slash commands are available (defined in `.opencode/commands/`). **If you do not use OpenCode**, ignore this table and invoke the same flows via chat + the `modes/` files (see “AI environment” above).

When using OpenCode, the following slash commands are available:
...
**Note:** OpenCode commands invoke the same `.claude/skills/career-ops/SKILL.md` skill used by Claude Code. The `modes/*` files are shared between both platforms. Cursor and other IDEs do not require this file: `CLAUDE.md` + `modes/` are enough.
