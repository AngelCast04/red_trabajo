...
Career-Ops convierte cualquier asistente de codigo (**Cursor**, Claude Code, OpenCode, Copilot, etc.) en un centro de mando de busqueda de empleo. **No necesitas un producto concreto**: basta un editor con IA que lea este repo y siga `CLAUDE.md` y `modes/`. En vez de trackear aplicaciones en un spreadsheet, tienes un pipeline AI que:
...
# 5. Personalizar con tu asistente (Cursor, Claude Code, etc.)
# Abre el proyecto en Cursor o el IDE que uses con IA.
...
> **El sistema esta diseñado para que el agente de codigo lo personalice** (Cursor, Claude Code, etc.). Modes, arquetipos, scoring, scripts de negociacion -- solo pidelo. El agente lee los mismos archivos que usa, asi que sabe exactamente que editar.
